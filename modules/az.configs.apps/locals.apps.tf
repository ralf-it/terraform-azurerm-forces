      ...
      for k, v in var.app_services_config : (v.role.app) => merge(v, {
        ...
        framework_configs = merge(
          try(v.framework_configs, {}),
          {
            ...
            app_settings_custom_from_url = distinct(sort([for x in flatten([
              try(v.framework_configs.app_settings_custom_from_url, []),
              try(var.app_services_framework_configs_per_customer[c][k]["app_settings_custom_from_url"], []),
              try(var.app_services_framework_configs_per_customer[c][v.role.app]["app_settings_custom_from_url"], []),
            ]) : x if x != null && x != ""]))
